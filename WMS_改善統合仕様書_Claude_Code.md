...
# 0. 追記（2026-09-13）：方針転換と現状のシステム内容

_この章は元の仕様書（2026-09-09作成）に対する追記であり、元の内容を上書き・削除するものではない。以降の開発はこの章の内容を新たな起点（baseline）として進める。_

## 0.1 新しい方針：InventorOSを完全に廃止し、Supabase単独構成へ

元の仕様書は「既存システムを捨てず、InventorOS（Laravel製の別システム）を参考にしながら改善する」という方針だったが、調査の結果、InventorOSは本番環境から一度も到達可能になったことがなく、実質的に機能していない依存先だった。

ユーザーとの協議の結果、以下の方針に転換した。

-   **InventorOSへの依存を完全に廃止する。** バックエンド（Laravel）・関連コード・オフライン同期キュー・未使用の画面フォルダをすべて削除した。
-   **Supabase（Postgres + Auth + Storage + Edge Functions）を唯一のバックエンドとする。** 2つのバックエンドが併存する状態（"半分が死んでいるアプリ"の最大の原因）を解消した。
-   InventorOSが持っていた業務モデル・UX・データ設計は「参考」として吸収済み（`domain_model.md`, `oss_reference_matrix.md`）だが、InventorOSのコード自体・VPSでの起動は不要であり、行わない。
-   `supabase_flutter` 等のSDKは使わず、PostgREST/RPC/Auth(GoTrue)/Storageへの生Dio呼び出しに統一する（このリポジトリのアーキテクチャ規約）。

詳細な削除内容・経緯は `migration_plan.md` の「Post-0028」の項を参照。

## 0.2 現状のシステム内容（2026-09-13時点、migration 0010〜0037適用済み）

元の仕様書 §46 のロードマップ番号に沿って、Step 1〜17 はほぼ完了しており、加えて元の仕様書には無かった以下の10項目を追加実装した（ユーザー指示「未実装のものを一つずつ確認して完成させる」に基づく完成パス）。

1.  画像/添付ファイルストレージ（検品への写真添付、非公開Storageバケット）
2.  商品マスタ（JANコードに紐づく商品名・カテゴリ・価格）
3.  発注（Purchase Orders）— 下書き→提出→承認/却下→完了のライフサイクル
4.  受注（Sales Orders）— 発注と対になる出庫側のライフサイクル
5.  仕入先/顧客CRM（`delivery_suppliers` を汎用取引先ディレクトリへ拡張）
6.  作業指示（Work Orders）— キッティング/組立、在庫を実際に消費・生産する唯一の新規機能
7.  カスタム/保存可能なレポート作成機能（6種類の安全なデータソース）

これに加え、認証（Supabase Auth）へのテストカバレッジ追加、ユーザー単位の倉庫アクセス範囲管理、AI（OCR）結果の人による確認画面なども、元の仕様書の意図（§3, §22, §27〜§31）を満たす形で今回のセッション中に実装済み。

現在実装済みの全機能・未実装のギャップの一覧は `docs/feature_checklist.md` を、システム構成の要約は `docs/architecture_current.md` を参照（この追記の内容と重複させないよう、詳細はそちらに一本化している）。

## 0.3 この時点を「最初」とすることについて

上記の方針転換とバックエンド統一により、本プロジェクトは実質的に「InventorOS依存のレガシー資産」を持たないクリーンな状態になった。したがって、**2026-09-13時点のシステム状態（migration 0037まで適用、`docs/feature_checklist.md` に記載の内容）を、今後の設計・仕様追加における新たな起点（第1版のベースライン）として扱う。** 元の仕様書 §1〜§53 の目的・方針自体は有効なまま引き継ぐが、「既存のInventorOS資産を吸収する」という前提部分（§2.1 など）は、InventorOS自体が既に存在しないため、今後は適用対象外とする。
