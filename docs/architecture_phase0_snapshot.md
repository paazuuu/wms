# Architecture — Phase 0 Snapshot (historical)

_Phase 0 investigation output. Snapshot of `paazuuu/wms` as it existed at the
very start of the spec-driven evolution (`WMS_改善統合仕様書_Claude_Code.md`),
kept for history. **This is not the current state** — see
`architecture_current.md` for today's actual architecture and
`feature_checklist.md` for a verified, feature-by-feature status. In short:
InventorOS (everything below describing it) was removed from the app entirely
— that backend was never actually reachable, and the decision was made not to
stand it up._
